...
var datePickerController = (function datePickerController() {
                ...
                if((curr > firstColIndex && curr <= (firstColIndex + dpm)) || o.fillGrid) {
                    ...
                    // This cells date is within the lower & upper ranges (or no ranges have been defined)
                        ...
                        if (currentDate in bespokeRenderClass) {
                            //anh.nguyen
                            if (bespokeRenderClass[currentDate] == 'day-enabled') {
                                var idx = cName.indexOf('day-disabled');
                                if (idx > -1) cName.splice(idx, 1);
                            }
                            //end
    ...
    datePicker.prototype.createButton = function() {
        ...
        //anh.nguyen
        but.href        = "#" + this.id;
        but.className = "btn btn-default btn-sm btn-datepicker";
    ...
    datePicker.prototype.setDateFromInput = function() {
        ...
        for(elemID in this.formElements) {
            ...
            if (!elemVal) {
                //anh.nguyen
                if (this.runCallBack) {
                    if (this.callBackFromCtr) this.callBackFromCtr();
                }
                else {
            ...
            for (i = 0; i < allFormats.length; i++) {
                ...
                dt = parseDateString(this.noYear?elemVal + "/2000":elemVal, allFormats[i]);
                //end
    ...
    datePicker.prototype.returnFormattedDate = function(noFocus) {
        ...
        for(elemID in this.formElements) {
            ...
            if (elem.tagName.toLowerCase() == "input") {
                //anh.nguyen
                if (this.noYear) {
                    this.dateSet.setYear(2000);
                    fmtDate = fmtDate.substr(0, 5);
                }
                elem.value = fmtDate;

                //anh.nguyen
                this.dateSet.setHours(0);
                if (this.callBackFromCtr) {
                    this.callBackFromCtr(this.dateSet, fmtDate);
                }
            } else {
    ...
    var addDatePicker = function(options) {
        ...
        for (elemID in options.formElements) {
            if (/\-[0-9]{6,}$/.test(elemID)) {
                tmpObj[elemID] = options.formElements[elemID];
                continue;
            }
            var e1 = document.getElementById(elemID);
            if (e1) {
                var newId = elemID + '-' + time;
                e1.id = newId;
                tmpObj[newId] = options.formElements[elemID];
            }
        }
        options.formElements = tmpObj;
        //end
        ...
        opts = {
            //anh.nguyen
            callBackFromCtr: options.callBack,
            ...
            highlightDays:options.highlightDays && options.highlightDays.length && options.highlightDays.length == 7 ? options.highlightDays : [0,0,0,0,0,0,0],//anh.nguyen
        ...
        //anh.nguyen
        if (options.datePickerObject) {
            options.datePickerObject(datePickers[options.id]);
        }
        //end
    ...
    return {
        ...
        //anh.nguyen
        createDatePicker: function (options) { setTimeout(function() { addDatePicker(options); }, 100); },
