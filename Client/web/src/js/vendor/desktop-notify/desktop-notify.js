...
(function (win) {
    ...
    function getNotification(title, options) {
        ...
        if (win.Notification) { /* Safari 6, Chrome (23+) */
            ...
            notification.callback = options.clickCallback;
            notification.group = options.group;
            notification.iconUrl = isString(options.icon) ? options.icon : options.icon.x32;
        ...
        if (options.group != null) {
            var list = {};
            for (var j in listNotifies) {
                var dicItem = listNotifies[j];
                if (dicItem.group == options.group) {
                    //close this
                    if (dicItem.close) {
                        //http://code.google.com/p/ff-html5notifications/issues/detail?id=58
                        dicItem.close();
                    }
                    else if (dicItem.cancel) {
                        dicItem.cancel();
                    } else if (win.external && win.external.msIsSiteMode()) {
                        if (dicItem.ieVerification === ieVerification) {
                            win.external.msSiteModeClearIconOverlay();
                        }
                    }
                    //add item to remove
                    list[dicItem.tag] = dicItem;
                }
            }

            for (var i in list) {
                delete listNotifies[i];
            }
        }
        listNotifies[notification.tag] = notification;
        return notification;
    }
    function getWrapper(notification) {
        return {
            close: function () {
                delete listNotifies[notification.tag];
                if (notification) {
            ...
            click: function () {
                ...
                //alway call active tab when click to notify
                window.focus();
            }
        };
    }
    ...
    function createNotification(title, options) {
        ...
        if (isSupported && isString(title) && (options && (isString(options.icon) || isObject(options.icon))) && (permissionLevel() === PERMISSION_GRANTED)) {
            notification = getNotification(title.toUpperCase(), options);
        }
        ...
        if (options.autoClose && notification && !notification.ieVerification && notification.addEventListener) {
