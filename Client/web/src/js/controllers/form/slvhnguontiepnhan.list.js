/*VIEW*/ if ($templateView.get('slvhnguontiepnhan.list') == null) $templateView.put('slvhnguontiepnhan.list', '<div id="slvhnguontiepnhanlist" ng-controller="slvhnguontiepnhan.list"><div ng-include src="\'text.search\'"></div><div ng-include src="\'f.add.csv\'"></div><div ag-grid="ModelData.ListSlvhnguontiepnhanOptions" ></div></div>');
function slvhnguontiepnhanList($scope, $location) {
        ...
        if ($scope.$parent.IsAuthenticated === false || $scope.$parent.IsSplashDone === false) {
                ...
                try {
                    ...
                    if (dataKey === RealtimeKey.Area) {
                        ...
                        if (entity.Status === MemberStatus.Active) {
                            $appUtil.dataSourceAddOrUpdateRealTimeItem($scope, $scope.ModelData.ListSlvhnguontiepnhanOptions.rowData, 'AreaId', entity.AreaId, entity);
                        } else {
                            $appUtil.dataSourceRemoveItem($scope, $scope.ModelData.ListSlvhnguontiepnhanOptions.rowData, 'AreaId', entity.AreaId);
                        }
            ...
            $scope.onResponseReceived = function (objectData) {
                ...
                if (objectData.Type === MsgResponse.GetListAreaResponse) {
                    ...
                    $scope.ModelData.ListSlvhnguontiepnhanOptions.rowData = listitem;
                    $scope.ModelData.ListSlvhnguontiepnhanOptions.api.setRowData();
                    $scope.ModelData.ListSlvhnguontiepnhanOptions.api.refreshView();
                }
            };
            ...
            $scope.textSearchChange = function () {
                $scope.ModelData.ListSlvhnguontiepnhanOptions.quickFilterText = $scope.textSearch;
            };
            ...
            $scope.ModelData.ListSlvhnguontiepnhanOptions = new AngularGridOptions($scope.ModelData.ListBasinColumns, Controllers.AreaList);
            $scope.ModelData.ListSlvhnguontiepnhanOptions.pinnedColumnCount = 2;
            $appUtil.initCount($scope, $scope.ModelData.ListSlvhnguontiepnhanOptions);
            //bắt đàu gửi request
            ...
            $scope.ModelData.GridSetting = function () {
                var windowInstant = $formCreator.createNewModalSetting("setting", $scope.ModelData.ListSlvhnguontiepnhanOptions.columnDefs);
                windowInstant.result.then(function (item) {
                    $scope.ModelData.ListSlvhnguontiepnhanOptions.columnDefs = item;
                    $scope.ModelData.ListSlvhnguontiepnhanOptions.api.setColumnDefs();
                });
            };
            //Export csv function
            $scope.ExportCSV = function () {
                var csvContent = '\ufeff' + $func.getDataCSV($scope.ModelData.ListSlvhnguontiepnhanOptions.columnDefs, $scope.ModelData.ListSlvhnguontiepnhanOptions.rowData);
            ...
            $scope.$on('$destroy', function () {
                ...
                $scope.ModelData.ListSlvhnguontiepnhanOptions = null;
