/*VIEW*/ if ($templateView.get('categories.type.river') == null) $templateView.put('categories.type.river', '<div id="categories" ng-controller="categories.type.river"><div ng-include src="\'text.search\'"></div><div ng-include src="\'f.add.csv\'"></div><div ag-grid="ModelData.ListCategoriesOptions" ></div></div>');
/*VIEW*/
if ($templateView.get('categories.type.river') == null) $templateView.put('categories.type.river', '<div id="categories" ng-controller="categories.type.river"><div ng-include src="\'text.search\'"></div><div ng-include src="\'f.add.csv\'"></div><div ag-grid="ModelData.ListCategoriesOptions" ></div></div>');
function categoriesTypeRiver($scope, $location) {
                ...
                if (listTempCategories != null && listTempCategories.length > 0) {
                    for (var j = 0; j < listTempCategories.length; j++) {
                        ...
                        for (var k = 0; k < listData.length; k++) {
                            ...
                            if (itemCategoriesType.TypeSource != TypeSource.River) continue;
            ...
            $scope.SaveFunctionClick = function () {
                ...
                for (var i = 0; i < $scope.ModelData.ListCategoriesOptions.rowData.length; i++) {
                    ...
                    if (itemCategories.minA || itemCategories.maxA) {
                        objCategoriesA.TypeSource = $appUtil.strToNum(TypeSource.River);
                    ...
                    if (itemCategories.minB || itemCategories.maxB) {
                        objCategoriesB.TypeSource = $appUtil.strToNum(TypeSource.River);
                    ...
                    if (itemCategories.minC || itemCategories.maxC) {
                        objCategoriesC.TypeSource = $appUtil.strToNum(TypeSource.River);
